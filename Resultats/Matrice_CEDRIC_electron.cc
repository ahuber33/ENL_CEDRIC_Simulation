#include "Matrice_CEDRIC_electron.hh"

void Matrice_CEDRIC_electron()
{
  Init_Table_Chi2();
  FillMatriceHP();
  TFile file1("CEDRIC_MatriceHP_electron.root", "RECREATE");
  MatriceHP->Write();

  FillMatrice();
  TFile file2("CEDRIC_Matrice_electron.root", "RECREATE");
  Matrice->Write();
  
  TGraph* Table_Chi2 = Table_Chi2_995pc;
  const int ddl =6;
  REF = Histo_PSL("Data_Electron_Exp/Electron_Exp_0.8MeV_100M.root");
  // TGraph* g;
  // g = Graph_Chi2_HP(REF);
  // g->Draw();
  ...
  new TCanvas;
  TGraphErrors* a = FOM_Spectro_FIT(0.2, 15, start_0, step_0, low_0, up_0, start_1, step_1, low_1, up_1);
