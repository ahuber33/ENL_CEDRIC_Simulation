...
void FillMatrice()
{
  ...
  for (int i=0; i<=200; i++)
    {
      sprintf(filename,"Data_Gamma_Uniform/Gamma_Uniform_%d_MeV_10M.root", i);
      Entries = 10000000.;
      if(Test_Fichier(filename) == false)
	{
	  sprintf(filename,"Data_Gamma_Uniform/Gamma_Uniform_%d_MeV_1M.root", i);
	  Entries = 1000000.;
...
void FillMatriceHP()
{
  ...
  for (float i=0; i<=100; i++)
    {
      a=i/10;
      sprintf(filename,"Data_Gamma_Uniform_HP/Gamma_Uniform_HP_%.1f_MeV_10M.root", a);
      Entries = 10000000.;
      cout << filename << endl;
      if(Test_Fichier(filename) == false)
	{
	  sprintf(filename,"Data_Gamma_Uniform_HP/Gamma_Uniform_HP_%.1f_MeV_1M.root", a);
	  Entries = 1000000.;
	  if(Test_Fichier(filename) == false) continue;
	}
      //      Entries = NumberOfEntries(filename);
...
TGraphAsymmErrors* FOM_Spectro(TGraph* alpha, int ddl, float Emin, float Emax, float step)
{
  ...
  vector<float>Largeur;
    ...
    {
      sprintf(filename,"Data_Gamma_Exp/Exp_%0.1fMeV_10M.root", num);
...
TGraphErrors* FOM_Spectro_FIT(float Emin, float Emax, float start_0, float step_0, float low_0, float up_0, float start_1, float step_1, float low_1, float up_1)
{
  ...
  float num = Emin;
    ...
    for (int i=0; i<npoints; i++)
    {
      ...
      if (E0 <=2)FIT_MINUIT_HP(start_0, step_0, low_0, up_0, start_1, step_1, low_1, up_1);
      x[i] = num;
      ex[i] = 0.1/2;
      y[i] = E0;
      ey[i] = err_E0;
